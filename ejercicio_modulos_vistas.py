from ejercicio_modulos_modelo import Abmc
...
class VistaPoo:
    def __init__(self, ventana_interfaz):
        ...
        #       def ventana_principal(self, main):
        self.main.title("PyNotes")
        self.main.config(bg="#FFF7DD")
        self.menubar = Menu(self.main)
        self.main.config(menu=self.menubar)
        self.menu_manual = Menu(self.menubar, tearoff=False)
        self.menubar.add_cascade(label="Ayuda", menu=self.menu_manual)

        self.menu_lenguaje = Menu(self.menubar, tearoff=False)
        self.menubar.add_cascade(label="Lenguaje", menu=self.menu_lenguaje)
